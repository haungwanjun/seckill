...
​			因为内存redis里面比DB要快，所以最好加各种各样的缓存，尽量少访问DB。 但是这样可能会引起页面显示内容有一点延迟。但是不是特别重要的场景的话。稍微一些延迟还是没有问题的，在可接受范围就好。

​	2、页面静态化，前后端分离

​			也就是把页面等静态资源缓存在客户端，这样前端和后端之间的交互就直选哟传输需要的参数就行。并不需要后端模板引擎吧页面渲染好然后把整个页面传到前端，极大的减少了服务器的压力和网络带宽的压力。
